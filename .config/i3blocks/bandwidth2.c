...
void usage(char *argv[])
{
  ...
  printf("-w Bytes:Bytes\tSet warning (color yellow) for Rx:Tx bandwidth. (default: none)\n");
             ...
             double b, int const warning, int const critical)
{
  ...
  if (critical != 0 && b > critical) {
    ...
    printf("<span fallback='true' color='%s'>", YELLOW);
  } else {
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    if (criticalrx != 0 && rx > criticalrx) {
      printf("<span color='%s'> </span>", RED);
    } else if (warningrx != 0 && rx > warningrx) {
      printf("<span color='%s'> </span>", YELLOW);
    } else {
      printf(" ");
    }
    display(unit, divisor, tx, warningtx, criticaltx);
    if (criticaltx != 0 && tx > criticaltx) {
      printf("<span color='%s'> </span>", RED);
    } else if (warningtx != 0 && tx > warningtx) {
      printf("<span color='%s'> </span>", YELLOW);
    } else {
      printf(" ");
    }
